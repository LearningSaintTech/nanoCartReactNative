...
const PartnerCartScreen = ({ navigation }) => {
  ...
  const cartTotal = cartItems.reduce(
    (sum, item) => sum + calculateTotalQty(item.orderDetails) * (item.itemId.MRP || 0),
    0,
  );
  console.log('Total items:', totalItems, 'Cart total (MRP-based):', cartTotal);
  ...
  useEffect(() => {
    ...
    const fetchInvoiceData = async () => {
      ...
      try {
        ...
        const calculatedCartTotal = cartTotal || 0;
        const discountedPrice = cartItems.reduce(
        ...
        if (
          ...
          const walletMoney =
            getLatestValue('wallet money') || appliedWalletAmount || 0;
          const couponDiscountValue = getLatestValue('coupon discount') || 0;
          const codCharges = getLatestValue('cod charges') || 0;
          const gstValue = getLatestValue('gst') || 0;
          const shippingCharges =
          ...
          console.warn('Invalid invoice data, using cart-based values');
          const walletMoney = appliedWalletAmount || 0;
          const couponDiscountValue = 0;
          const codCharges = 0;
          const gstValue = 0;
          const shippingCharges = 0;
    ...
    try {
      ...
      if (response.ok && data.success) {
        setInvoiceData(prev => {
          const newInvoiceData = {
            ...prev,
            couponDiscount: (data.discount || 0).toFixed(2),
            totalAmount: (
              (parseFloat(prev.discountedPrice) || 0) -
              (parseFloat(prev.walletMoney) || 0) -
              (data.discount || 0) +
              (parseFloat(prev.codCharges) || 0) +
              (parseFloat(prev.gst) || 0)
            ).toFixed(1),
            savings: (
              (parseFloat(prev.cartTotal) || 0) -
              (parseFloat(prev.discountedPrice) || 0) +
              (data.discount || 0) +
              (parseFloat(prev.walletMoney) || 0)
            ).toFixed(2),
        ...
        Alert.alert(
          'Success',
          `Coupon applied! ₹${data.discount || 0} discount added.`,
        );
      } else {
        console.error('Failed to apply coupon:', data.message);
        Alert.alert('Error', data.message || 'Invalid coupon code.');
  ...
  const handleContinue = () => {
    ...
    const navigationData = {
      ...
      cartTotal,
      appliedWalletAmount,
      couponDiscount: parseFloat(invoiceData.couponDiscount) || 0,
      cartItems,
      invoiceData,
    };
    console.log('Navigating to PartnerDelivery with data:', navigationData);
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 150 }}>
        ...
        {cartItems.map((item, index) => {
          ...
          const totalQty = calculateTotalQty(item.orderDetails);
          const pricePerUnit = item.totalPrice / (item.totalQuantity || 1);
          const totalPriceItem = calculateTotalPrice(
            item.orderDetails,
            pricePerUnit,
          );
            ...
            <View key={item._id} style={styles.card}>
              <View style={styles.row}>
                ...
                <View style={{ flex: 1, marginLeft: 10 }}>
                  ...
                  <Text style={styles.category}>{item.itemId.description}</Text>
                  <Text style={styles.price}>
                    MRP <Text style={styles.mrp}>₹{item.itemId.MRP || 0}</Text>{' '}
                    <Text style={{ fontWeight: 'bold' }}>
                      ₹{pricePerUnit.toFixed(2)}
                    </Text>
                  </Text>
        ...
        <View style={styles.walletContainer}>
          ...
          <Text style={styles.balanceText}>
            {walletLoading
              ? 'Loading...'
              : `Available Balance (₹${walletBalance || 0})`}
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  balanceText: { fontSize: 13, color: '#666',padding:10 },
  priceBox: {
    backgroundColor: '#FDF6F1',
    padding: 12,
    marginHorizontal: 12,
