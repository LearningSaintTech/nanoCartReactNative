...
const SubCategoryScreen = ({ navigation, route }) => {
  ...
  const [filters, setFilters] = useState({}); // Track applied filters
      ...
      .then((json) => {
        if (json.success && json.data.items) {
          const formatted = json.data.items.map((item) => ({
            ...
            itemId: item._id,
            userAverageRating: item.userAverageRating,
          }));
  ...
  const handleApplyFilters = (filteredItems, filterState, pagination) => {
    setProducts(filteredItems);
    setFilters(filterState); // Update filters state
    setIsFiltered(true); // Prevent fetchItems from running
    setListKey(Date.now().toString());
