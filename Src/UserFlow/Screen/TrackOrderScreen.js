...
const TrackOrderScreen = ({ route, navigation }) => {
  ...
  const getStatusSteps = (status) => {
    const steps = [
      ...
      { label: 'Out for delivery', completed: false, date: null }, // Update with dispatch date if available
  ...
  console.log('Order Payment Statuss zaiba  hereeee :', order.paymentStatus, 'Order Status:', order.orderStatus);
...
const styles = StyleSheet.create({
  ...
  back: { fontSize: 20, marginRight: 8 ,marginTop:30},
  headerTitle: { fontWeight: 'bold', fontSize: 16 ,marginTop:30},
  ...
  returnStatus: { fontSize: 12, color: '#D6722F', marginTop: 4 },
  ...
  returnInfo: { marginTop: 8, paddingLeft: 10 },
  returnText: { fontSize: 12, color: '#555', marginBottom: 4 },
  viewDetailsButton: {
    borderWidth: 1,
    borderColor: '#D6722F',
    borderRadius: 4,
