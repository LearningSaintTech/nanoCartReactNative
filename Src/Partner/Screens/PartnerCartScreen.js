...
const PartnerCartScreen = ({ navigation }) => {
  console.log('PartnerCartScreen rendered at:', new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' }));
  ...
  const calculateTotalPrice = (orderDetails, pricePerUnit) => {
    ...
    console.log('Calculated total price:', total, 'with pricePerUnit:', pricePerUnit);
    return total;
  };
  ...
  useEffect(() => {
    console.log('useEffect triggered at:', new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' }));
    console.log('useEffect dependencies - token:', token, 'cartItems.length:', cartItems.length, 'appliedWalletAmount:', appliedWalletAmount);
    ...
    const fetchCartDetails = async () => {
      ...
      try {
        ...
        const response = await fetch(`${BASE_URL}/api/partner/cart`, {
          method: 'GET',
          headers: {
        ...
        if (response.ok && data.success && Array.isArray(data.data.items)) {
          ...
          const stockMap = stockResponses.reduce((acc, json, idx) => {
            if (json.data && Array.isArray(json.data) && json.data[0]) {
              acc[data.data.items[idx].itemId._id] = json.data[0].imagesByColor || [];
            }
            return acc;
          }, {});
          ...
          console.error('Failed to fetch cart:', data.message || 'Invalid response');
          dispatch(clearCart());
    ...
    const fetchInvoiceData = async () => {
      ...
      try {
        ...
        const discountedPrice = cartItems.reduce((sum, item) => sum + item.totalPrice, 0);
        console.log('Calculated cartTotal:', calculatedCartTotal, 'discountedPrice:', discountedPrice);
  ...
  const toggleExpand = (index) => {
    console.log('toggleExpand called for index:', index, 'current expandedIndex:', expandedIndex);
  ...
  const handleQuantityChange = async (itemId, color, size, newQuantity, itemIndex) => {
    console.log('handleQuantityChange called:', { itemId, color, size, newQuantity, itemIndex });
    ...
    if (itemStockData) {
      ...
      if (colorData) {
        ...
        if (sizeData && newQuantity > sizeData.stock) {
          console.log(`Quantity ${newQuantity} exceeds stock ${sizeData.stock} for size ${size} in ${color}`);
          Alert.alert(
            'Error',
  ...
  const handleRemoveItem = async (item, itemIndex) => {
    console.log('handleRemoveItem called for item:', item.itemId.name, 'at index:', itemIndex);
    try {
      const payload = {
      ...
      if (response.ok && data.success) {
        ...
        console.log('Dispatched setCartItems after removing item at index:', itemIndex);
        Alert.alert('Success', `${item.itemId.name} has been removed from the cart.`);
      } else {
        console.error('Failed to remove item:', data.message);
        Alert.alert('Error', data.message || 'Failed to remove the item from the cart.');
      }
    } catch (error) {
  ...
  const handleMoveToWishlist = async (item, itemIndex) => {
    console.log('handleMoveToWishlist called for item:', item.itemId.name, 'at index:', itemIndex);
    try {
      const payload = {
        itemId: item.itemId._id,
  ...
  const handleApplyWallet = () => {
    ...
    if (walletBalance === null) {
      ...
      Alert.alert('Error', 'Wallet balance is not available. Please try again later.');
      return;
    }

    if (enteredAmount > walletBalance) {
      console.log(`Entered amount ${enteredAmount} exceeds wallet balance ${walletBalance}`);
      Alert.alert(
        'Error',
    ...
    console.log('Applied wallet amount:', enteredAmount, 'isWalletApplied:', true);
    Alert.alert('Success', `Wallet amount of ₹${enteredAmount} has been applied.`);
  };
  ...
  const handleRemoveWallet = () => {
    ...
    console.log('Removed wallet amount, appliedWalletAmount:', 0, 'isWalletApplied:', false);
      ...
      if (response.ok && data.success) {
        setInvoiceData((prev) => {
          const newInvoiceData = {
            ...
            savings: (
              (parseFloat(prev.cartTotal) - parseFloat(prev.discountedPrice)) +
              data.discount +
              parseFloat(prev.walletMoney)
            ).toFixed(2),
          };
        ...
        Alert.alert('Success', `Coupon applied! ₹${data.discount} discount added.`);
      } else {
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 150 }}>
        ...
        {cartItems.map((item, index) => {
          console.log('Rendering cart item:', item.itemId.name, 'at index:', index);
          const totalQty = calculateTotalQty(item.orderDetails);
          const pricePerUnit = item.totalPrice / item.totalQuantity;
          const totalPriceItem = calculateTotalPrice(item.orderDetails, pricePerUnit);
          ...
          const selectedColorObj = item.orderDetails
            ...
            : null;
          console.log('Item details - totalQty:', totalQty, 'pricePerUnit:', pricePerUnit, 'selectedColorIndex:', selectedColorIndex);
              ...
              <View style={styles.row}>
                ...
                <View style={{ flex: 1, marginLeft: 10 }}>
                  ...
                  <Text style={styles.price}>
                    ...
                    <Text style={{ fontWeight: 'bold' }}>₹{pricePerUnit.toFixed(2)}</Text>
                  </Text>
                </View>
              </View>
                ...
                <View style={styles.orderDetailsContent}>
                  <View style={styles.colorBoxesRow}>
                    {item.orderDetails.map((colorObj, i) => {
                      ...
                      console.log('Rendering color box for color:', colorObj.color, 'hexCode:', hexCode);
                      return (
                        <TouchableOpacity
                          key={i}
                          onPress={() => setSelectedColorIndices((prev) => {
                            const newIndices = { ...prev, [index]: i };
                            console.log('Selected color index updated:', newIndices);
                            return newIndices;
                          })}
                          style={[
                            styles.colorBox,
                  ...
                  <View>
                    {selectedColorObj.sizeAndQuantity.map((sizeObj, j) => {
                      ...
                      const sizeData = colorData?.sizes.find((s) => s.size === sizeObj.size);
                      const isOutOfStock = sizeData && sizeData.stock === 0;
                      console.log('Rendering size:', sizeObj.size, 'quantity:', sizeObj.quantity, 'isOutOfStock:', isOutOfStock);
                      ...
                      return (
                        <View key={j} style={styles.sizeRow}>
                          <Text
                            style={[
                              styles.sizeText,
                              isOutOfStock && { textDecorationLine: 'line-through', color: '#888' },
                            ]}
                          >
                  ...
                  <View style={styles.totalContainer}>
                    ...
                    <View style={styles.totalRow}>
                      ...
                      <Text style={styles.totalValue}>₹{pricePerUnit.toFixed(2)}</Text>
                      <Text style={styles.totalValue}>₹{totalPriceItem.toLocaleString('en-IN')}</Text>
                    </View>
                  </View>
                </View>
              )}
        ...
        <View style={styles.couponContainer}>
          <TouchableOpacity
            onPress={() => {
              console.log('Toggling coupon accordion, current state:', couponAccordionOpen);
        ...
        <View style={styles.walletContainer}>
          ...
          <View style={styles.couponInputContainer}>
            ...
            <TouchableOpacity
              ...
              onPress={isWalletApplied ? handleRemoveWallet : handleApplyWallet}
            >
              <Text style={styles.applyBtnText}>{isWalletApplied ? 'REMOVE' : 'APPLY'}</Text>
            </TouchableOpacity>
          </View>
          <Text style={styles.balanceText}>
            {walletLoading ? 'Loading...' : `Available Balance (₹${walletBalance})`}
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  emptyCartText: { fontSize: 18, color: '#666', textAlign: 'center', marginTop: 20 },
  stepRow: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  paymentMethod: { paddingHorizontal: 16, fontSize: 15, marginVertical: 8, color: '#333' },
  continueBtn: {
