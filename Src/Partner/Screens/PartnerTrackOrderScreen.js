...
const TrackOrderScreen = ({ route, navigation }) => {
  ...
  const getStatusSteps = (status) => {
    ...
    if (['Processing', 'Confirmed', 'Ready for Dispatch', 'In transit', 'Dispatched', 'Delivered'].includes(status)) {
      steps[0].completed = true;
    }
    if (['Confirmed', 'Ready for Dispatch', 'In transit', 'Dispatched', 'Delivered'].includes(status)) {
      steps[1].completed = true;
    }
    if (['Ready for Dispatch', 'In transit', 'Dispatched', 'Delivered'].includes(status)) {
  ...
  const isReturnInProgress = order?.isOrderReturned && order?.returnInfo?.refundStatus === 'Processing';
  ...
  const isReturnEligible = () => {
    ...
    return currentDate <= maxReturnDate && order.orderStatus === 'Delivered' && !isReturnInProgress;
  };
  ...
  const getItemTotalQuantity = (item) => {
    return item.orderDetails.reduce(
      (sum, detail) => sum + detail.sizeAndQuantity.reduce((q, s) => q + s.quantity, 0),
      0
    );
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.header}>
        <TouchableOpacity onPress={() => navigation.navigate('PartnerOrderHistoryScreen')}>
      ...
      {/* Order Confirmation */}
      <View style={[styles.section, order.orderStatus === 'In transit' && styles.inTransitSection]}>
        ...
        <Text style={styles.statusDesc}>
          {order.orderStatus === 'Processing' && 'Your order is currently being processed.'}
          {order.orderStatus === 'Confirmed' && 'Your order has been confirmed by our team.'}
          {order.orderStatus === 'Ready for Dispatch' && 'Your order is ready for dispatch and will be shipped soon.'}
          {order.orderStatus === 'In transit' && 'Your order is on its way to the delivery address.'}
          {order.orderStatus === 'Dispatched' && 'Your order has been dispatched from our facility.'}
          {order.orderStatus === 'Delivered' && 'Your order has been successfully delivered.'}
          {order.orderStatus === 'Order Returned' && 'A return request has been initiated. Awaiting pickup and processing.'}
        </Text>
      ...
      {isReturnInProgress && (
        <View style={styles.section}>
          ...
          <View style={styles.returnInfo}>
            <Text style={styles.returnText}>Return Status: {order.returnInfo?.refundStatus || 'N/A'}</Text>
            <Text style={styles.returnText}>Reason: {order.returnInfo?.reason || 'N/A'}</Text>
            <Text style={styles.returnText}>Requested: {formatDate(order.returnInfo?.requestDate)}</Text>
            <Text style={styles.returnText}>
              Refund Amount: ₹{order.returnInfo?.refundAmount?.toFixed(2) || 'N/A'}
            </Text>
            {order.returnInfo?.pickupLocation && (
              <Text style={styles.returnText}>
                ...
                {order.returnInfo.pickupLocation.cityTown}, {order.returnInfo.pickupLocation.state},{' '}
      ...
      <View style={styles.subSection}>
        <Text style={[styles.subTitle, styles.beigeTitle]}>Order Details</Text>
            ...
            <View style={styles.productInfo}>
              ...
              {item.orderDetails.map((detail, idx) => (
                <Text key={idx} style={styles.detail}>
                  Color: {detail.color} | Size: {detail.sizeAndQuantity.map(s => s.size).join(', ')}
                </Text>
              ))}
              <Text style={styles.detail}>Qty: {getItemTotalQuantity(item)}</Text>
              <View style={styles.priceContainer}>
                <Text style={styles.priceLabel}>MRP </Text>
                <Text style={styles.strike}>₹{item.itemId.MRP}</Text>
                <Text style={styles.discountedPrice}> ₹{item.itemId.discountedPrice}</Text>
                <Text style={styles.savingsText}>
                  {calculateSavingsPercentage(item.itemId.MRP, item.itemId.discountedPrice)}% OFF
                </Text>
              </View>
                ...
                <View style={styles.timelineIcon}>
                  <View
                    style={[
                      ...
                      step.completed ? styles.iconBackgroundCompleted : styles.iconBackgroundPending,
                    ]}
                  />
                  <View
                    style={[
                      styles.iconForeground,
                      step.completed ? styles.iconForegroundCompleted : styles.iconForegroundPending,
                    ]}
                  />
                </View>
      ...
      {/* Delivery Address */}
      <View style={styles.subSection}>
        <Text style={[styles.subTitle, styles.beigeTitle]}>Delivery Address</Text>
        {order.shippingAddress ? (
          <View>
            <Text style={styles.addressName}>{order.shippingAddress.name || 'Unknown'}</Text>
      ...
      <View style={styles.subSection}>
        ...
        <View style={styles.billRow}>
          ...
          <Text style={styles.billValue}>
            ₹{order.invoice.find((entry) => entry.key === 'carttotal')?.values || '0.00'}
          </Text>
        </View>
        <View style={styles.billRow}>
          <Text style={styles.billLabel}>Discounted Price</Text>
          <Text style={styles.billValue}>
            ₹{order.invoice.find((entry) => entry.key === 'discountedprice')?.values || '0.00'}
          </Text>
        </View>
        <View style={styles.billRow}>
          <Text style={styles.billLabel}>Wallet Money</Text>
          <Text style={styles.billValue}>
            -₹{order.invoice.find((entry) => entry.key === 'walletmoney')?.values || '0.00'}
          </Text>
        </View>
        <View style={styles.billRow}>
          <Text style={styles.couponLabel}>COUPON DISCOUNT</Text>
          <Text style={styles.couponValue}>
            -₹{order.invoice.find((entry) => entry.key === 'coupondiscount')?.values || '0.00'}
          </Text>
        </View>
        <View style={styles.billRow}>
          <Text style={styles.billLabel}>COD CHARGES</Text>
          <Text style={styles.billValue}>
            ₹{order.invoice.find((entry) => entry.key === 'codcharges')?.values || '0.00'}
          </Text>
        </View>
        <View style={styles.billRow}>
          <Text style={styles.billLabel}>GST</Text>
          <Text style={styles.billValue}>
            ₹{order.invoice.find((entry) => entry.key === 'gst')?.values || '0.00'}
          </Text>
        </View>
        <View style={styles.billRow}>
          <Text style={styles.billLabel}>Shipping Charges</Text>
          <Text style={styles.billValue}>
            {parseFloat(order.invoice.find((entry) => entry.key === 'shippingcharges')?.values || 0) === 0
              ? 'FREE'
              : `₹${order.invoice.find((entry) => entry.key === 'shippingcharges')?.values || '0.00'}`}
          </Text>
        </View>
        ...
        <Text style={styles.savings}>
          ...
          {order.invoice.find((entry) => entry.key === 'savings')?.values || '0.00'}/- WITH THIS ORDER!
        </Text>
