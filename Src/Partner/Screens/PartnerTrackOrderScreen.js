...
const TrackOrderScreen = ({ route, navigation }) => {
  ...
  const fetchAddress = async () => {
    try {
      ...
      setAddresses(responseData.addresses.addressDetail);
    } catch (err) {
      console.error('Error fetching address:', err.message);
      setAddressError(
        err.message.includes('401')
          ? 'Session expired. Please log in again.'
          : 'Failed to load address. Please check your network and try again.',
      );
      if (err.message.includes('401')) navigation.navigate('Login');
    } finally {
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.subSection}>
          ...
          {addressLoading ? (
            <View style={styles.loadingContainer}>
              <ActivityIndicator size="small" color="#D6722F" />
              <Text style={styles.loadingText}>Loading address...</Text>
            </View>
          ) : addressError ? (
            <View>
              <Text style={styles.errorText}>{addressError}</Text>
              <TouchableOpacity style={styles.retryButton} onPress={fetchAddress}>
                <Text style={styles.retryButtonText}>Retry</Text>
              </TouchableOpacity>
            </View>
          ) : defaultAddress ? (
            <View>
              <Text style={styles.addressName}>
                {defaultAddress.name || 'Unknown'}
              </Text>
              <Text style={styles.address}>
                {defaultAddress.addressLine1 || ''},{' '}
                {defaultAddress.addressLine2 || ''},{' '}
                {defaultAddress.cityTown || 'N/A'},{' '}
                {defaultAddress.state || 'N/A'},{' '}
                {defaultAddress.pincode || 'N/A'}
              </Text>
            </View>
          ) : (
            <Text style={styles.address}>No delivery address available.</Text>
