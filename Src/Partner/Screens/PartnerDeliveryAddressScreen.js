...
const PartnerDeliveryAddressScreen = ({navigation}) => {
  ...
  const [loadingAddress, setLoadingAddress] = useState(true);
  const [invoiceData, setInvoiceData] = useState({
    cartTotal: navigationData?.invoiceData?.cartTotal || '0.00',
    discountedPrice: navigationData?.invoiceData?.discountedPrice || '0.00',
    walletMoney: navigationData?.invoiceData?.walletMoney || '0.00',
    couponDiscount: navigationData?.invoiceData?.couponDiscount || '0.00',
    codCharges: navigationData?.invoiceData?.codCharges || '0.00',
    gst: navigationData?.invoiceData?.gst || '0.0',
    shippingCharges: navigationData?.invoiceData?.shippingCharges || 'FREE',
    totalAmount: navigationData?.invoiceData?.totalAmount || '0.0',
    savings: navigationData?.invoiceData?.savings || '0.00',
  });
    ...
    const fetchInvoiceData = async () => {
      ...
      try {
        ...
        if (
          ...
          // Calculate totalAmount including shippingCharges
          const totalAmount =
            discountedPrice -
            walletMoney -
            couponDiscountValue +
